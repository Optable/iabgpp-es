...
export class TcfEuV2CoreSegment extends AbstractLazilyEncodableSegment {
    ...
    toObj() {
        return {
            Version: this.getFieldObj(TcfEuV2Field.VERSION),
            Created: this.getFieldObj(TcfEuV2Field.CREATED),
            LastUpdated: this.getFieldObj(TcfEuV2Field.LAST_UPDATED),
            CmpId: this.getFieldObj(TcfEuV2Field.CMP_ID),
            CmpVersion: this.getFieldObj(TcfEuV2Field.CMP_VERSION),
            ConsentScreen: this.getFieldObj(TcfEuV2Field.CONSENT_SCREEN),
            ConsentLanguage: this.getFieldObj(TcfEuV2Field.CONSENT_LANGUAGE),
            VendorListVersion: this.getFieldObj(TcfEuV2Field.VENDOR_LIST_VERSION),
            TcfPolicyVersion: this.getFieldObj(TcfEuV2Field.POLICY_VERSION),
            IsServiceSpecific: this.getFieldObj(TcfEuV2Field.IS_SERVICE_SPECIFIC),
            UseNonStandardTexts: this.getFieldObj(TcfEuV2Field.USE_NON_STANDARD_STACKS),
            SpecialFeatureOptins: this.getFieldObj(TcfEuV2Field.SPECIAL_FEATURE_OPTINS),
            PurposeConsent: this.getFieldObj(TcfEuV2Field.PURPOSE_CONSENTS),
            PurposesLITransparency: this.getFieldObj(TcfEuV2Field.PURPOSE_LEGITIMATE_INTERESTS),
            PurposeOneTreatment: this.getFieldObj(TcfEuV2Field.PURPOSE_ONE_TREATMENT),
            PublisherCC: this.getFieldObj(TcfEuV2Field.PUBLISHER_COUNTRY_CODE),
            VendorConsent: this.getFieldObj(TcfEuV2Field.VENDOR_CONSENTS),
            VendorLegitimateInterest: this.getFieldObj(TcfEuV2Field.VENDOR_LEGITIMATE_INTERESTS),
            PubRestrictions: this.getFieldObj(TcfEuV2Field.PUBLISHER_RESTRICTIONS),
        };
    }
    // overriden
    initializeFields() {
